...
const ComponentDisplay = ({ component, setComponent }: props) => {
	...
	const handleUpdateComponent = (
		callBack: (component: TComponent) => TComponent,
	) => {
		setComponent((prev) => ({
			...prev,
	...
	const handleRemoveRelationship = (id1: number, id2: number) => {
		setComponent((prev) => ({
			...
			relationships: prev.relationships.filter(
				(relationship) =>
					!(
						(relationship.firstComponentId === id1 &&
							relationship.secondComponentId === id2) ||
						(relationship.secondComponentId === id1 &&
							relationship.firstComponentId === id2)
					),
			),
		}));
	};
	...
	return (
		<div className="flex flex-col size-full">
			...
			<div className="flex p-6">
				<button
					type="button"
					className="bg-green-400 w-full border rounded-md py-4 "
			...
			<div className="flex flex-1 size-full p-4 border rounded-lg shadow-md bg-white relative overflow-auto">
				{component.children.map((child) => (
					<ChildDisplay
						key={child.id}
						component={child}
						onClick={() => setEditedComponent(child)}
					/>
				))}
				{component.relationships.map((relationship) => (
					<RelationshipDisplay
						key={
							relationship.firstComponentId +
							"_" +
							relationship.secondComponentId
						}
						relationship={relationship}
						firstComponent={component.children.find(
							(c) => c.id === relationship.firstComponentId,
						)}
						secondComponent={component.children.find(
							(c) => c.id === relationship.secondComponentId,
						)}
					/>
				))}
			</div>
			...
			{editedComponent && (
				<ComponentEditorModal
					...
					relationshipIds={getIdsOfRelatedComponents(
						editedComponent.id,
						component.relationships,
					)}
					siblings={component.children.filter(
						(child) => child.id !== editedComponent.id,
					)}
